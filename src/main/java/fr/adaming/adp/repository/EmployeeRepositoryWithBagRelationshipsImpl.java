...
public class EmployeeRepositoryWithBagRelationshipsImpl implements EmployeeRepositoryWithBagRelationships {
    ...
    public Optional<Employee> fetchBagRelationships(Optional<Employee> employee) {
        return employee.map(this::fetchJobs);
    }
    ...
    public List<Employee> fetchBagRelationships(List<Employee> employees) {
        return Optional.of(employees).map(this::fetchJobs).orElse(Collections.emptyList());
    }

    Employee fetchJobs(Employee result) {
        return entityManager
            .createQuery("select employee from Employee employee left join fetch employee.jobs where employee is :employee", Employee.class)
            .setParameter("employee", result)
            .setHint(QueryHints.PASS_DISTINCT_THROUGH, false)
            .getSingleResult();
    }

    List<Employee> fetchJobs(List<Employee> employees) {
        ...
        List<Employee> result = entityManager
            .createQuery(
                "select distinct employee from Employee employee left join fetch employee.jobs where employee in :employees",
                Employee.class
            )
